...
class PriorityQueue(Queue):
    ...
    lt) is returned first.  Also supports changing the priority of
    an element already in the queue (the queue will be resorted).
    The constructor takes two arguments:
        f: a function that takes an element of the queue and returns a number,
           which is the priority of that element (default: identity function)
        lt: a binary predicate that returns true if its first argument has
            higher priority than its second (default: <)
    Note that this PriorityQueue is not the same as Python's Queue.PriorityQueue.
    """
    ...
    def append(self, item):
        self.A.append(item)
        # Ordena de mayor a menor para que pop() saque el de menor f(item)
